...
# --- 2. وظائف مساعدة (مع الإصلاح النهائي) ---
...
def calculate_indicators(df, strategy):
    try:
        if df is None or df.empty or len(df) < 20: # زيادة طفيفة للاحتياط
            return None, None
        
        df.ta.ema(length=strategy['ema_period'], append=True)
        df.ta.rsi(length=strategy['rsi_period'], append=True)
        
        # الإصلاح بناءً على سجل الخطأ: تحديد 3 أسماء للأعمدة
        stoch_cols = [f'STOCHk_{strategy["stoch_k"]}_{strategy["stoch_d"]}_{strategy["stoch_smooth_k"]}', 
                      f'STOCHd_{strategy["stoch_k"]}_{strategy["stoch_d"]}_{strategy["stoch_smooth_k"]}',
                      f'STOCHh_{strategy["stoch_k"]}_{strategy["stoch_d"]}_{strategy["stoch_smooth_k"]}'] # اسم العمود الثالث الإضافي
        df.ta.stoch(k=strategy['stoch_k'], d=strategy['stoch_d'], smooth_k=strategy['stoch_smooth_k'], append=True, col_names=stoch_cols)
        
        df.ta.atr(length=14, append=True)
        
        return df.iloc[0], df.iloc[1]
    except Exception as e:
        logger.error(f"CRITICAL ERROR in calculate_indicators: {e}\n{traceback.format_exc()}")
        return None, None

# --- 4. منطق تحليل السوق (مبسط بعد الإصلاح) ---
...
async def market_analysis_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = await update.message.reply_text("⏳ جاري تحليل السوق...")
    ...
    if not active_pairs_found:
        await msg.edit_text("تحليل السوق اكتمل: لم يتم العثور على أزواج نشطة.")
        return
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text("أهلاً بك في بوت النصيري.", reply_markup=reply_markup)
...
async def start_bot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not bot_state['active_pairs']:
        await update.message.reply_text("❌ اختر زوج واحد على الأقل أولاً.")
        return
...
async def settings_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [[InlineKeyboardButton("حساسية ATR", callback_data="set_atr")], [InlineKeyboardButton("🔄 استعادة الافتراضي", callback_data="reset_strategy")]]
...
async def set_atr(update: Update, context: ContextTypes.DEFAULT_TYPE): await ask_for_input(update, context, 'atr_sensitivity', "أرسل **حساسية ATR** (مثال: 0.0005):")
...
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    except (ValueError, IndexError): await update.message.reply_text(f"❌ إدخال خاطئ.")
