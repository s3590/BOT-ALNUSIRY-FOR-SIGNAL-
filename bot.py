...
def calculate_indicators(df):
    ...
    try:
        ...
        df.ta.stoch(k=s['stoch_k'], d=s['stoch_d'], smooth_k=s['stoch_smooth_k'], append=True, col_names=('STOCHk', 'STOCHd', 'STOCHs'))
...
def check_strategy(data):
    ...
    # 1. EMA
    ema_val = latest_data.get('EMA')
    if ema_val is not None and not pd.isna(ema_val):
        if close_price > ema_val: signals['buy'].append('EMA')
        if close_price < ema_val: signals['sell'].append('EMA')

    # 2. RSI
    rsi_val = latest_data.get('RSI')
    if rsi_val is not None and not pd.isna(rsi_val):
        if rsi_val < s['rsi_oversold']: signals['buy'].append('RSI')
        if rsi_val > s['rsi_overbought']: signals['sell'].append('RSI')

    # 3. Stochastic
    stoch_k = latest_data.get('STOCHk')
    stoch_d = latest_data.get('STOCHd')
    if stoch_k is not None and stoch_d is not None and not pd.isna(stoch_k) and not pd.isna(stoch_d):
        is_oversold = stoch_k < s['stoch_oversold'] and stoch_d < s['stoch_oversold']
        is_overbought = stoch_k > s['stoch_overbought'] and stoch_d > s['stoch_overbought']
        
        # الشرط: تقاطع صاعد في منطقة تشبع البيع
        if is_oversold and stoch_k > stoch_d:
            signals['buy'].append('Stochastic')
        # الشرط: تقاطع هابط في منطقة تشبع الشراء
        if is_overbought and stoch_k < stoch_d:
...
async def strategy_settings_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    target_message = update.callback_query.message if hasattr(update, 'callback_query') and update.callback_query else update.message
    try:
        if hasattr(update, 'callback_query') and update.callback_query:
            await target_message.edit_text(text, reply_markup=keyboard, parse_mode='Markdown')
        else:
            await target_message.reply_text(text, reply_markup=keyboard, parse_mode='Markdown')
    except Exception as e:
        logger.error(f"Error sending/editing settings menu: {e}")
...
def main() -> None:
    ...
    logger.info("Starting Telegram Bot Polling (Web Service Mode)...")
