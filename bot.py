...
def calculate_indicators(df, strategy):
    if df is None or df.empty or len(df) < strategy['ema_period']:
        return None, None
    try:
...
async def market_analysis_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # **إرسال التقرير التشخيصي إلى تليجرام**
    await context.bot.send_message(chat_id=TELEGRAM_CHAT_ID, text=f"`{volatility_report}`", parse_mode='MarkdownV2')
...
# ... (بقية الكود الخاص بالأزرار والإعدادات يبقى كما هو)
